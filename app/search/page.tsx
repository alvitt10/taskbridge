            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {CATEGORIES.map(c => (
                <button key={c} onClick={() => setFilters(f => ({ ...f, category: c }))}
                  style={{ padding: '7px 16px', borderRadius: 999, fontSize: 13, cursor: 'pointer', border: `1px solid ${filters.category === c ? 'var(--accent)' : 'var(--border)'}`, background: filters.category === c ? 'var(--accent-light)' : 'white', color: filters.category === c ? 'var(--accent)' : 'var(--muted)', fontWeight: filters.category === c ? 600 : 400 }}>
                  {c}
                </button>
              ))}
            </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 48px', display: 'grid', gridTemplateColumns: '260px 1fr', gap: 32, alignItems: 'start' }}>
          <aside style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 16, padding: 24, position: 'sticky', top: 84 }}>
            ...
            <FilterGroup label="Max Distance">
              <input type="range" min={1} max={50} value={filters.radius} onChange={e => setFilters(f => ({ ...f, radius: Number(e.target.value) }))} style={{ width: '100%', accentColor: 'var(--accent)' }} />
            ...
            <FilterGroup label="Max Price ($/hr)">
              <input type="range" min={20} max={200} step={5} value={filters.maxPrice} onChange={e => setFilters(f => ({ ...f, maxPrice: Number(e.target.value) }))} style={{ width: '100%', accentColor: 'var(--accent)' }} />
            ...
            <FilterGroup label="Min Rating">
              {[{ v: 0, l: 'Any' }, { v: 4, l: '4.0+' }, { v: 4.5, l: '4.5+' }, { v: 4.8, l: '4.8+ ⭐ Top' }].map(r => (
                <label key={r.v} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', fontSize: 14, color: 'var(--muted)', cursor: 'pointer' }}>
                  <input type="radio" name="rating" checked={filters.minRating === r.v} onChange={() => setFilters(f => ({ ...f, minRating: r.v }))} style={{ accentColor: 'var(--accent)' }} />
                  {r.l}
                </label>
              ))}
            </FilterGroup>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
                {providers.map(p => (
                  <ProviderCard key={p.id} provider={p as any} onBook={() => setSelectedProvider(p as any)} />
                ))}
              </div>
            )}
          </div>
